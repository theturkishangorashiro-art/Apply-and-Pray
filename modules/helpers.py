...
def get_default_temp_profile() -> str:
    '''Returns a writable Chrome profile folder for temporary browser sessions.'''
    home = pathlib.Path.home()
    if sys.platform.startswith('win'):
        local_app_data = os.environ.get('LOCALAPPDATA')
        base = pathlib.Path(local_app_data) if local_app_data else home / 'AppData' / 'Local'
        return str(base / 'ApplyAndPrayChromeProfile')
    elif sys.platform.startswith('linux'):
        return str(home / '.apply-and-pray-chrome-profile')
    return str(home / 'Library' / 'Application Support' / 'Google' / 'Chrome' / 'ApplyAndPrayChromeProfile')
